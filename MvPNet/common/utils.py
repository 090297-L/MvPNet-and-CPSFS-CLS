...
def detect_grad_nan(model):
    for param in model.parameters():
        if param.grad is not None:  # 确保参数有梯度
            if torch.isnan(param.grad).sum() != 0:  # 检测是否存在NaN
...
def parse_args(arg_mode):
    ...
    parser.add_argument('-data_dir', type=str, default='F:/python_code/few-shot-classification/IISNet-main/datasets/', help='dir of datasets')

    ''' about training specs '''
    parser.add_argument('-batch', type=int, default=16, help='auxiliary batch size')
    ...
    parser.add_argument('-extra_dir', type=str, default='FSC_cross', help='extra dir name added to checkpoint dir')
