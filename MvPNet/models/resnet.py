...
class ResNet_change(nn.Module):

    def __init__(self, args, block=BasicBlock):
        self.inplanes = 3
        super(ResNet_change, self).__init__()
        ...
        self.conv640t320_1 = nn.Conv2d(640, 320, kernel_size=1)
        self.conv640t320_2 = nn.Conv2d(640, 320, kernel_size=1)
    ...
    def forward(self, x):
        ...
        x_up = self.conv640t320_1(x)
        x_up = F.interpolate(x_up, scale_factor=2, mode='bilinear')
        x = torch.cat([x1, x_up], dim=1)
        x = self.conv640t320_2(x)
